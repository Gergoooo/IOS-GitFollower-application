...
class FavoritesListVS: DPDataLoadingVC {
    ...
    func updateUI(with favorites: [Follower]){
        if favorites.isEmpty {
            self.showEmptyStateView(message: "No favorites?\nAdd one ont the follower screen.", in: self.view)
        } else{
...
extension FavoritesListVS: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        ...
        PersistenceManager.updateWith(favorite: favorites[indexPath.row], actionTYpe: .remove) { [weak self] error in
            ...
            self.presentDPAlertOnMainThread(title: "Unable to remove", message: error.rawValue, buttonTitle: "OK")
        }
    }
}
